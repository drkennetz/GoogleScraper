...
// select a random userAgent for each of our requests
...
// Builds google URLs for us. It'll be a proper query for google to run (like a link)
...
func GoogleScrape(searchTerm, languageCode, countryCode string, proxyString interface{}, pages, count, waiter int)([]SearchResult, error) {
	results := []SearchResult{}
	resultCounter := 0
	...
	for _, page := range googlePages {
		...
		time.Sleep(time.Duration(waiter) * time.Second)
	}
	return results, nil
}

func main() {
	searchQuery := flag.String("query", "random", "executes a google search")
	pages := flag.Int("pages", 1, "the number of pages to return")
	count := flag.Int("count", 30, "the count to return")
	flag.Parse()
	res, err := GoogleScrape(*searchQuery, "en", "com", nil, *pages, *count, 10)
	if err != nil {
		log.Fatalln("There was an error, exiting.")
	}
